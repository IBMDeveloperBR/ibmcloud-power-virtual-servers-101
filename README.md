# IBM Power Systems Virtual Servers on IBM Cloud

[![service-card](./images/service-card.png)](https://cloud.ibm.com/catalog/services/power-systems-virtual-server)

## Book Contents

- [1. Introduction](./chapters/chapter-1.md)
    - [1.1. Why use IBM Power Systems?](./chapters/chapter-1.md#11-why-use-ibm-power-systems)
    - [1.2. Underlying infrastructure environment](./chapters/chapter-1.md#12-underlying-infrastructure-environment)
    - [1.3. Available Cloud Regions and Zones](./chapters/chapter-1.md#13-available-cloud-regions-and-zones)
    - [1.4. Network options](./chapters/chapter-1.md#14-network-options)
    - [1.5. Storage options](./chapters/chapter-1.md#15-storage-options)
    - [1.6. Compute options](./chapters/chapter-1.md#16-compute-options)
    - [1.7. Available software](./chapters/chapter-1.md#17-available-software)
- [2. Backup and migration strategies for IBM Power VS](./chapters/chapter-2.md)
    - [2.1. Backup strategies](./chapters/chapter-2.md#21-backup-strategies-for-ibm-power-virtual-servers)
    - [2.2. Migration strategies](./chapters/chapter-2.md#22-migration-strategies-for-ibm-power-virtual-servers)
    - [2.3. Planning workload migrations to POWER8 and POWER9](./chapters/chapter-2.md#23-planning-workload-migrations-to-power8-and-power9)
- [3. High Availability and Disaster Recovery options for IBM Power VS](./chapters/chapter-3.md)

## Hands-On Labs

- [LAB 1: Provisioning and managing Power Virtual Servers](./labs/lab-1/README.md)
- [LAB 2: Capturing and exporting AIX and IBM i VM images](./labs/lab-2/README.md)
- [LAB 3: Deploying custom images within a Power Systems Virtual Server](./labs/lab-3/README.md)
- [LAB 4: Snapshotting, cloning and restoring Power Virtual Servers](./labs/lab-4/README.md)
- [LAB 5: Direct Link Connect for Power Systems Virtual Servers](./labs/llab-5/README.md)

## References

- [IBM Cloud Power Virtual Servers documentation](https://cloud.ibm.com/docs/power-iaas)
- [IBM Cloud Power Virtual Servers offering web page](https://www.ibm.com/cloud/power-virtual-server)
